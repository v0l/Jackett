...
namespace Jackett.Common.Indexers.Definitions
{
    ...
    public class Nostr : IndexerBase
    {
        ...
        /// </summary>
        private static int? MapLegacyTagsToCategory(List<string> tags) => MapCategory(string.Join(",", tags));
        ...
        private static (string? Tcat, string[]? Hashtags) MapCategoryToFilter(int cat)
        {
            ...
            if (cat == TorznabCatType.Movies.ID || cat == TorznabCatType.MoviesForeign.ID ||
                cat == TorznabCatType.MoviesOther.ID || cat == TorznabCatType.MoviesSD.ID ||
                cat == TorznabCatType.MoviesBluRay.ID || cat == TorznabCatType.Movies3D.ID ||
            ...
            // TV - hashtag fallback
            if (cat == TorznabCatType.TV.ID || cat == TorznabCatType.TVWEBDL.ID || cat == TorznabCatType.TVForeign.ID ||
                cat == TorznabCatType.TVSD.ID || cat == TorznabCatType.TVOther.ID || cat == TorznabCatType.TVSport.ID ||
            ...
            if (cat == TorznabCatType.AudioMP3.ID || cat == TorznabCatType.AudioVideo.ID ||
            ...
            // PC - hashtag fallback
            if (cat == TorznabCatType.PC.ID || cat == TorznabCatType.PC0day.ID || cat == TorznabCatType.PCISO.ID ||
            ...
            if (cat == TorznabCatType.ConsolePSP.ID || cat == TorznabCatType.ConsolePS3.ID ||
                cat == TorznabCatType.ConsolePSVita.ID || cat == TorznabCatType.ConsolePS4.ID)
                return ("games,psx", null);
            if (cat == TorznabCatType.ConsoleWii.ID || cat == TorznabCatType.ConsoleWiiware.ID ||
                cat == TorznabCatType.ConsoleWiiU.ID)
                return ("games,wii", null);
            if (cat == TorznabCatType.ConsoleXBox.ID || cat == TorznabCatType.ConsoleXBox360.ID ||
                cat == TorznabCatType.ConsoleXBoxOne.ID)
                return ("games,xbox", null);

            // Console - hashtag fallback
            if (cat == TorznabCatType.Console.ID || cat == TorznabCatType.ConsoleNDS.ID ||
                cat == TorznabCatType.ConsoleOther.ID || cat == TorznabCatType.Console3DS.ID)
                return (null, new[]
                {
                    "games"
                });
            ...
            if (cat == TorznabCatType.XXX.ID || cat == TorznabCatType.XXXx264.ID || cat == TorznabCatType.XXXUHD.ID ||
                cat == TorznabCatType.XXXPack.ID || cat == TorznabCatType.XXXOther.ID || cat == TorznabCatType.XXXSD.ID ||
                cat == TorznabCatType.XXXWEBDL.ID || cat == TorznabCatType.XXXDVD.ID)
                return (null, new[]
                {
                    "xxx"
                });
            ...
            if (cat == TorznabCatType.Books.ID || cat == TorznabCatType.BooksMags.ID ||
                cat == TorznabCatType.BooksTechnical.ID || cat == TorznabCatType.BooksOther.ID ||
            ...
            if (cat == TorznabCatType.Other.ID || cat == TorznabCatType.OtherMisc.ID || cat == TorznabCatType.OtherHashed.ID)
                return (null, new[]
                {
                    "other"
                });
